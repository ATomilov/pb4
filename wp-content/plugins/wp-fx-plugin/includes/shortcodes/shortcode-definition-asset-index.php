...
function ol_asset_index_shortcode($atts)
{
    ...
    if (isset($the_query->posts[0])) {
        ...
        foreach ($assets as $asset) {
            $asset = (object) $asset;
            if (in_array($asset->name, $uniqueAssets)) {
                continue;
            }
        ...
        if ($overridden_template = locate_template('wp-fx-plugin/assets-main.php')) {
            load_template($overridden_template);
        } else {
            load_template(PLUGIN_DIR . '/templates/assets-index/assets-main.php');
        }
